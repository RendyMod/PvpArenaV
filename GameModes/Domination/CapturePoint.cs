...
namespace PvpArena.GameModes.Domination
{
	...
	namespace PvpArena.Models
	{
		...
		public class CapturePoint
		{
			...
			private const float breakpointCaptureDelay = 1.25f; // Time to capture from one state to another
			...
			public CapturePoint(RectangleZone zone, int pointIndex)
			{
				Zone = zone;
				timer = new Stopwatch();
				timer.Start();
			...
			public void Update(List<Player> team1Players, List<Player> team2Players, Action<int> awardPointsCallback)
			{
				if (!DominationGameMode.MatchActive) return;
				...
				if (controllingTeamId != 0)
				{
					...
					{
						awardPointsCallback(controllingTeamId); // Award points to controlling team
