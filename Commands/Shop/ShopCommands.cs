...
internal static class ShopCommands
{
	...
	{
		sender.ReceiveMessage($"You have {sender.PlayerPointsData.GetPointsFromCurrentRegion().ToString().Emphasize()} {"VPoint(s)".Warning()}".White());
	}
	...
	public static void AddPointsCommand(Player sender, int points, Player foundPlayer = null)
	{
		...
		sender.ReceiveMessage(($"{player.Name.Colorify(ExtendedColor.ClanNameColor)} {"VPoint(s)".Warning()} are now {player.PlayerPointsData.GetPointsFromCurrentRegion().ToString().Emphasize()}.").White());
		
		player.ReceiveMessage($"An admin added {points.ToString().Emphasize()} {"VPoint(s)".Warning()} to you.".Success());
		player.ReceiveMessage(($"Your {"VPoint(s)".Warning()} are now {player.PlayerPointsData.GetPointsFromCurrentRegion().ToString().Emphasize()}.").White());
	}
	...
	{
		sender.ReceiveMessage($"{player.Name.Colorify(ExtendedColor.ClanNameColor)} has {player.PlayerPointsData.GetPointsFromCurrentRegion().ToString().Emphasize()} {"VPoint(s)".Warning()}".White());
	}
}
